...
using RetroGame;
using RetroGame.Screen.ExtraClassForGames.Asteroid;
#endregion
...
namespace RetroGame.Screen
{
    class AsteroidGameScreen : GameScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            //ship's texture
            ship = new Sprite(content.Load<Texture2D>("asteroidAssets/ship"));
            
            //shot's texture
            shot = new Sprite(content.Load<Texture2D>("asteroidAssets/shot"));

            //large asteroids
            for (int i = 1; i < 4; i++)
                asteroidTexture.Add(content.Load<Texture2D>(
                    "asteroidAssets/asteroids/large" + i.ToString()));

            //medium asteroids
            for (int i = 1; i < 4; i++)
                asteroidTexture.Add(content.Load<Texture2D>(
                    "asteroidAssets/asteroids/medium" + i.ToString()));

            //small asteroids
            for (int i = 1; i < 4; i++)
                asteroidTexture.Add(content.Load<Texture2D>(
                    "asteroidAssets/asteroids/small" + i.ToString()));

            //Font
            scoreFont = content.Load<SpriteFont>("asteroidAssets/asteroids/asteroidFont");
